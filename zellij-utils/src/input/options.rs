...
pub struct Options {
    ...
    /// Command for automatic mouse-selection copies; falls back to copy_command or OSC52.
    /// Receives selected text on stdin. Only used when copy_on_select is enabled.
    #[clap(long, value_parser)]
    #[serde(default)]
...
impl Options {
    ...
    pub fn merge(&self, other: Options) -> Options {
        ...
        let copy_on_select_command = other
            .copy_on_select_command
            .or_else(|| self.copy_on_select_command.clone());
...
mod tests {
    ...
    #[test]
    fn copy_on_select_command_merge_preserves_or_overrides_independently() {
        let config = Options {
            copy_command: Some("clipboard".into()),
            copy_on_select_command: Some("clipboard --autogenerated".into()),
            ..Default::default()
        };
        for from_cli in [false, true] {
            let merge = |other| {
                if from_cli {
                    config.merge_from_cli(other)
                } else {
                    config.merge(other)
                }
            };
            let inherited = merge(Options::default());
            assert_eq!(
                inherited.copy_on_select_command,
                config.copy_on_select_command
            );
            let overridden = merge(Options {
                copy_on_select_command: Some("selection-writer".into()),
                ..Default::default()
            });
            assert_eq!(
                overridden.copy_on_select_command.as_deref(),
                Some("selection-writer")
            );
            assert_eq!(overridden.copy_command, config.copy_command);
        }
        assert_eq!(Options::default().copy_on_select_command, None);
    }
