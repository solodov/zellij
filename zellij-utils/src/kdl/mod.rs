...
impl Options {
    pub fn from_kdl(kdl_options: &KdlDocument) -> Result<Self, ConfigError> {
        ...
        let copy_on_select_command =
            kdl_property_first_arg_as_string_or_error!(kdl_options, "copy_on_select_command")
    ...
    /// Serialize the optional mouse-copy override without changing explicit copy behavior.
    fn copy_on_select_command_to_kdl(&self, add_comments: bool) -> Option<KdlNode> {
        let command = self.copy_on_select_command.as_deref()?;
        let mut node = KdlNode::new("copy_on_select_command");
        node.push(command.to_owned());
        if add_comments {
            node.set_leading(
                "\n// Command for automatic mouse-selection copies; receives text on stdin.\n\
                 // Requires copy_on_select. When unset, uses copy_command or OSC52.\n\
                 // Explicit Copy actions still use copy_command or OSC52.\n",
            );
        }
        Some(node)
    }
...
// Note: This only applies to web clients at the moment."#;
    ...
    pub fn to_kdl(&self, add_comments: bool) -> Vec<KdlNode> {
        ...
        if let Some(command) = self.copy_on_select_command_to_kdl(add_comments) {
            nodes.push(command);
        }
...
#[test]
fn copy_on_select_command_kdl_round_trip() {
    let document: KdlDocument = r#"
        copy_command "clipboard"
        copy_on_select_command "clipboard --autogenerated"
        copy_on_select true
    "#
    .parse()
    .unwrap();
    let parsed = Options::from_kdl(&document).unwrap();
    assert_eq!(parsed.copy_command.as_deref(), Some("clipboard"));
    assert_eq!(
        parsed.copy_on_select_command.as_deref(),
        Some("clipboard --autogenerated")
    );
    for add_comments in [false, true] {
        let mut serialized = KdlDocument::new();
        *serialized.nodes_mut() = parsed.to_kdl(add_comments);
        let reparsed = Options::from_kdl(&serialized.to_string().parse().unwrap()).unwrap();
        assert_eq!(parsed, reparsed);
    }
    let unset = Options::from_kdl(&KdlDocument::new()).unwrap();
    assert_eq!(unset.copy_on_select_command, None);
    let invalid: KdlDocument = "copy_on_select_command true".parse().unwrap();
    assert!(Options::from_kdl(&invalid).is_err());
}
